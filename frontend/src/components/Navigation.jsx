      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
        <Link to="/" data-testid="nav-logo" className="flex items-center gap-2 group">
          <span className="font-display text-2xl sm:text-3xl tracking-wide text-white leading-none">
            PATRICKGOMES<span className="text-[#CCFF00]">FR</span>
          </span>
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm font-semibold text-zinc-300">
          {isHome ? (
            <>
              <a href="#produto" className="hover:text-[#CCFF00] transition-colors" data-testid="nav-produto">
                Produto
              </a>
              <a href="#vantagens" className="hover:text-[#CCFF00] transition-colors" data-testid="nav-vantagens">
                Vantagens
              </a>
              <a href="#atletas" className="hover:text-[#CCFF00] transition-colors" data-testid="nav-atletas">
                Atletas
              </a>
            </>
          ) : (
        ...
        <button
          ...
          className="relative flex items-center gap-2 rounded-full border border-white/15 hover:border-[#CCFF00] px-4 py-2 transition-colors"
        >
          ...
          {count > 0 && (
            <span
              ...
              className="absolute -top-2 -right-2 bg-[#CCFF00] text-black text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center"
            >
              {count}
            </span>
          )}
